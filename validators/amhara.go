...
func VerifyAmharaBank(parsedData *models.AmharaBankParsedData, flags models.VerificationFlags) (bool, error) {
	if parsedData == nil {
		return false, utils.NewValidationError("No parsed data for Amhara Bank")
	}

	if parsedData.Status != "Auth" {
		return false, utils.NewValidationError("Transaction status is not successful")
	}
	...
	if len(date) < 6 {
		return false, utils.NewValidationError("Invalid booking date format")
	}
	...
	expected := config.Config.AmharaBank.Expected
